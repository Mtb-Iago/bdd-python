...
def simular_escolha_personagem(foto_personagem):
    foto = {
        "foto": foto_personagem
    }
    return foto

def reconhecer_personagem(foto_personagem_simulado):
    reconhecido = False
    mensagem    = "Personagem não reconhecido na base de dados, Entrada negada!!!"

    personagens_permitidos = simular_escolha_personagem(foto_personagem_simulado)
...
def verificar_permissao_zion(reconhecido, mensagem, dados):
    retorno = False
    print("verificando permissão para entrar em zion: ")
    reconhecido, mensagem, dados
    if reconhecido:
        print(mensagem)
        
        if dados['permissao'] == True:
            retorno = liberar_entrada_zion(dados)
            
        else:
            retorno = negar_entrada_zion(dados)
    else:
        print(mensagem)
